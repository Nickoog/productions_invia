...
/* jshint -W062 */
var SonicUi = SonicUi || {},
	SonicAjaxNav = SonicAjaxNav || {},
	SonicParams = SonicParams || {},
	console = console || {};

SonicAjaxNav = function( $ ) {
	...
	return {
		...
		setAjaxLinkClass : function () {
			...
			var siteURL = SonicParams.siteUrl,
				$internalLinks,
				regEx = '';

			$.each( SonicParams.allowedMimeTypes, function( index, value ) {
				regEx += '|' + value;
			} );

			regEx = $.trim( regEx ).substring(1);

			siteURL = SonicParams.siteUrl;
			...
			if ( SonicParams.isWooCommerce ) {

				/*
				When WC pages aren't set the WC pages variables will return the siteURL
				Be sure it is not the same !!
				 */
				if ( this.untrailingSlashit( siteURL ) !== this.untrailingSlashit( SonicParams.WooCommerceCartUrl ) ) {
					$( "a[href^='" + SonicParams.WooCommerceCartUrl + "']" ).removeClass( 'ajax-link' );
				}

				if ( this.untrailingSlashit( siteURL ) !== this.untrailingSlashit( SonicParams.WooCommerceAccountUrl ) ) {
					$( "a[href^='" + SonicParams.WooCommerceAccountUrl + "']" ).removeClass( 'ajax-link' );
				}

				if ( this.untrailingSlashit( siteURL ) !== this.untrailingSlashit( SonicParams.WooCommerceCheckoutUrl ) ) {
					$( "a[href^='" + SonicParams.WooCommerceCheckoutUrl + "']" ).removeClass( 'ajax-link' );
				}
			...
			$( '#blog-filter a, #albums-filter a, #videos-filter a, #plugin-filter a, .logged-in-as a, #trigger a' ).removeClass( 'ajax-link' );
		...
		goToAnchor : function ( href ) {
			...
			var hash,
				scrollPoint = 0,
				scrollOffset = SonicUi.getToolBarOffset() + SonicUi.getMenuOffsetFromTheme() - 5;
		...
		navigate : function ( href, clicked ) {
			...
			currentUrl = window.location.href;
			newHref = href.replace( href, SonicParams.siteUrl );
			...
			randStart = parseInt( Math.random() * ( 88 - 8 ) + 8 );
			...
			$.ajax( {
				...
				type : 'POST',
				dataType : 'html',
				url: SonicParams.ajaxUrl,
				data : {
					url : href,
					action : 'sonic_ajax_get_page_markup',
					nonce: nonce
				},
		...
		loadCustomPostStyle : function() {
			var $postStyle = this.$dom.find( 'style#sonic-single-post-style-inline-css' );
			$( 'style#sonic-single-post-style-inline-css' ).html( $postStyle.html() ); // replace single post style inline CSS
		},
		...
		themeCallback : function () {
			...
			if ( 'undefined' !== typeof SonicUi ) {
				SonicUi.init();
				SonicUi.postBg();
				SonicUi.singlePostNavBg();
				SonicUi.fluidVideos();
				...
				SonicUi.owlCarousel();
				SonicUi.parallax();
				SonicUi.postPaddingTop();
				SonicUi.animateAnchorLinks();
				SonicUi.shareLinkPopup();
				SonicUi.lazyLoad();
				SonicUi.additionalFixes();
			}
			...
			if ( 'undefined' !== typeof SonicMasonry ) {
				SonicMasonry.init();
			}

			if ( 'undefined' !== typeof SonicCategoryFilter ) {
				SonicCategoryFilter.init();
			}

			if ( 'undefined' !== typeof SonicInfiniteScroll ) {
				SonicInfiniteScroll.init();
			}

			if ( 'undefined' !== typeof SonicPhotos ) {
				SonicPhotos.init();
			}
		...
		WooCommercePriceFilter : function () {
			...
			if ( typeof woocommerce_price_slider_params === 'undefined' ||  1 > $( '.price_slider' ).length ) {
				return false;
			}
