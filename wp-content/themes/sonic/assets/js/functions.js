...
var SonicUi = SonicUi || {},
	...
	WolfFrameworkJSParams = WolfFrameworkJSParams || {},
	console = console || {};
...
SonicUi = function( $ ) {
	...
	return {
		...
		logoMenuPadding : function () {
			if ( $( '.logo a img' ).length && 'centered' === SonicParams.menuLayout ) {
		...
		stickyMenu : function ( scrollTop ) {
			...
			if ( ! SonicParams.isStickyMenu ) {
				return;
			}

			var menuType = this.getMenuType(),
				scrollPoint;

			scrollTop = scrollTop || 0;

			scrollPoint = parseInt( SonicParams.stickyMenuScrollPoint, 10 );
		...
		breakPoint : function () {
			...
			var winWidth = $( window ).width(),
				breakpoint = SonicParams.breakPoint;
		...
		setVimeoOptions : function() {
			...
			if ( SonicParams.accentColor ) {
				accentColor = SonicParams.accentColor.replace( '#', '' );
			} else {
				accentColor = '007acc';
			}
		...
		lightbox : function() {
			...
			if ( SonicParams.doWoocommerceLightbox ) {
			...
			if ( SonicParams.doVideoLightbox ) {

				if ( 'swipebox' === SonicParams.lightbox ) {

					$( '.video-item .entry-link, .lightbox-video' ).swipebox( {
						hideBarsDelay : 0,
						vimeoColor : SonicParams.accentColor,
						afterOpen : function () {
							_this.setVimeoOptions();
						}
					} );

				} else if ( 'fancybox' === SonicParams.lightbox ) {
				...
				if ( videoItem.length ) {
					...
					videoItem.each( function() {
						...
						data = {
							action: 'sonic_ajax_get_video_url_from_post_id',
							id : postId
						};

						$.post( SonicParams.ajaxUrl , data, function(response){
		...
		commentForm : function () {
			...
			$( '#comment' ).attr( 'placeholder', SonicParams.l10n.replyTitle );
		...
		smoothScroll : function( href ) {
			var body = this.body,
				scrollOffset = this.getToolBarOffset() + this.getMenuOffsetFromTheme() - 5,
				hash;
		...
		shareLinkPopup : function () {
			...
			$( '.share-link, .share-link-video' ).on( 'click', function() {
		...
		likes : function () {
			...
			$item.each( function () {
				if ( $.cookie( SonicParams.themeSlug + '-likes-' +  $( this ).data( 'post-id' ) ) ) {
			...
			$item.on( 'click', '.likes-meta', function() {
				...
				if ( $this.hasClass( 'liked' ) || $.cookie( SonicParams.themeSlug + '-likes-' + postId ) ) {
					...
					$.post( SonicParams.ajaxUrl, data, function( response ) {
						...
						$.cookie( SonicParams.themeSlug + '-likes-' + postId, 'liked', { path: '/', expires: 5 } );
					} );
				}
		...
		topLinkAnimation : function( scrollTop ){
			...
			if ( SonicParams.doBackToTopAnimation ) {
				if ( scrollTop >= 550 ) {
					$( 'a#back-to-top' ).show();
				} else {
					$( 'a#back-to-top' ).hide();
		...
		updateWooCommerceCart : function () {
			...
			if ( $( '.cart-menu-item' ).length ) {
				...
				if ( $.cookie( SonicParams.themeSlug + '_woocommerce_items_in_cart' ) ) {
					bubble.html( $.cookie( SonicParams.themeSlug + '_woocommerce_items_in_cart' ) );
					panelCount.html( $.cookie( SonicParams.themeSlug + '_woocommerce_items_in_cart' ) );
				}

				if ( $.cookie( SonicParams.themeSlug + '_woocommerce_cart_total' ) ) {
					amount.html( $.cookie( SonicParams.themeSlug + '_woocommerce_cart_total' ) );
				}
			}
		},
		 ...
		 */
		bigText404 : function () {
			if ( $( 'body' ).hasClass( 'error404' ) && SonicParams.hasWPB ) {
				...
				if ( 8 === event.keyCode || 46 === event.keyCode ) {
					...
					timer = setTimeout( function() {
						...
						var data = {
							...
							action : 'sonic_ajax_woocommerce_live_search',
							s : term
						};
