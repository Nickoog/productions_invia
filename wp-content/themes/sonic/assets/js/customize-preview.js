...
;( function( $ ) {
	 ...
	 * Background
	 */
	var style = $( '#sonic-color-scheme-css' ),
		api = wp.customize,
		backgrounds = {
	...
	if ( ! style.length ) {
		style = $( 'head' ).append( '<style type="text/css" id="sonic-color-scheme-css" />' )
		                    .find( '#sonic-color-scheme-css' );
	}
	...
	api( 'portfolio_layout', function( value ) {
		value.bind( function( to ) {
			...
			if ( $( 'body' ).hasClass( 'portfolio-layout-masonry' ) ) {
				$( '#content' ).isotope( 'reloadItems' ).isotope();
			}
			$( window ).trigger( 'resize' );
		} );
