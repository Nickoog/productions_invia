...
class Wolf_Theme_Customizer {
	...
	public function register_sections( $wp_customize ) {
		...
		foreach ( $this->sections as $section ) {
			...
			$title = isset( $section['title'] ) ? $section['title'] : esc_html__( 'Section Title', 'sonic' );
	...
	public function background_setting( $option, $section_id, $wp_customize, $section = true, $priority = 35 ) {
		...
		if ( $font_color ) {
			...
			$wolf_wp_customize_color_control = new WP_Customize_Color_Control(
				...
				$background_id . '_font_color',
					array(
						'label' => esc_html__( 'Font Color', 'sonic' ),
						'section' => $section_id,
		...
		if ( $is_no_bg ) {
			...
			$wp_customize->add_control(
				...
				array(
					'type' => 'checkbox',
					'label' => esc_html__( 'No Background', 'sonic' ),
					'section' => $section_id,
					'priority' => 1,
				)
			);
		...
		if ( $opacity ) :
			...
			$wp_customize->add_control(
				$background_id . '_opacity',
				array(
					'label' => esc_html__( 'Background Color Opacity (in percent)', 'sonic' ),
					'section' => $section_id,
					'type' => 'text',
					'priority' => 3,
				)
		...
		if ( $is_bg_img ) :
			...
			$wolf_wp_customize_image_control = new WP_Customize_Image_Control(
				...
				$background_id . '_img',
					array(
						'label' => esc_html__( 'Background Image', 'sonic' ),
						'section' => $section_id,
			...
			$wp_customize->add_control(
				...
				array(
					'type' => 'select',
					'label' => esc_html__( 'Background Repeat', 'sonic' ),
					'section' => $section_id,
					'choices' => array(
						'no-repeat' => 'no-repeat',
						'repeat' => 'repeat',
			...
			$wp_customize->add_control(
				...
				array(
					'type' => 'select',
					'label' => esc_html__( 'Background Position', 'sonic' ),
					'section' => $section_id,
					'choices' => array(
			...
			$wp_customize->add_control(
				...
				array(
					'type' => 'select',
					'label' => esc_html__( 'Background Attachment', 'sonic' ),
					'section' => $section_id,
					'choices' => array(
						'scroll' => 'scroll',
						'fixed' => 'fixed',
			...
			$wp_customize->add_control(
				...
				array(
					'type' => 'select',
					'label' => esc_html__( 'Background Size', 'sonic' ),
					'section' => $section_id,
					'choices' => array(
						'cover' => esc_html__( 'Cover', 'sonic' ),
						'contain' => esc_html__( 'Contain', 'sonic' ),
						'100% auto' => esc_html__( '100% width', 'sonic' ),
						'auto 100%' => esc_html__( '100% height', 'sonic' ),
						'inherit' => esc_html__( 'Inherit', 'sonic' ),
					),
					'priority' => 8,
				)
			);
			...
			if ( $parallax ) {
				...
				$wp_customize->add_control(
					...
					array(
						'type' => 'checkbox',
						'label' => esc_html__( 'Parallax', 'sonic' ),
						'section' => $section_id,
						'priority' => 9,
					)
				);
