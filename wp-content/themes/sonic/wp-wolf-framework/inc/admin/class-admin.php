...
if ( ! class_exists( 'Wolf_Framework_Admin' ) ) {
	...
	class Wolf_Framework_Admin {
		...
		public function menu() {
			add_theme_page( esc_html__( 'Theme Options', 'sonic' ), esc_html__( 'Theme Options', 'sonic' ), 'manage_options', 'wolf-theme-options', array( $this, 'options' ) );
			add_theme_page( esc_html__( 'Theme Updates', 'sonic' ), esc_html__( 'Theme Updates', 'sonic' ), 'manage_options', 'wolf-theme-update', array( $this, 'update_page' ) );
		}
		...
		public function display_notice() {
			...
			$wrong_install = sprintf(
				wp_kses( 
					__( 'It seems that <strong>the theme has been installed incorrectly</strong>. Go <a href="%s" target="_blank">here</a> to find instructions about theme installation.', 'sonic' ),
			...
			$wordpress_version_too_old = sprintf(
				wp_kses( 
					__( 'It seems that <strong>your WordPress installation is a bit old</strong>. It is recommended to <a href="%s">update your installation</a> to the latest version.', 'sonic' ),
					array( 'strong' => array(), 'a' => array( 'href' => array(), 'target' => array() ) )
					),
		...
		public function default_mods() {
			...
			if ( false !== ( $data= @unserialize( $file_content ) ) ) {
				...
				$mods = apply_filters( 'wolf_default_mods', $mods ); // filter default mods id needed
