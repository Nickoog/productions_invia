...
if ( ! class_exists( 'Wolf_Theme_Admin_Options' ) ) {
	...
	class Wolf_Theme_Admin_Options {
		...
		public function save() {
			...
			// 	wolf_admin_notice( esc_html__( 'Your options have been saved.', 'sonic' ), 'success' );
			// }

			// if ( isset( $_GET['message'] ) && 'export' == $_GET['message'] && isset( $_GET['page'] ) && $_GET['page'] == 'wolf-theme-options' ) {
			// 	wolf_admin_notice( esc_html__( 'Your download should start in a few seconds.', 'sonic' ), 'success' );
			// }
			...
			if ( isset( $_GET['page'] ) && $_GET['page'] == 'wolf-theme-options' ) {
					...
					foreach ( $this->options as $value ) {
						...
						elseif ( 'email' == $type ) {
							...
							if ( ! empty( $data[ $value_key ] ) && ! is_email( $data[ $value_key ] ) ) {
								...
								$errors[] = '<strong>' . $data[ $value_key ] . '</strong> '.esc_html__( 'is not a valid email', 'sonic' ).'.';
				...
				if ( ( isset( $_POST['action'] ) ) && 'wolf-reset-all-options' == $_POST['action'] ) {
					wolf_admin_notice( esc_html__( 'Your options have been reset.', 'sonic' ), 'success' );
				}
		...
		public function render() {
			...
			if ( is_child_theme() && WOLF_UPDATE_NOTICE )
				$theme_version = sprintf( esc_html__( 'v.%1$s (Parent Theme v.%2$s)', 'sonic' ), wp_get_theme()->Version, WOLF_THEME_VERSION ) ;
	...
	<?php foreach ( $this->options as $value ) {
		...
		?>
			<div class="wolf-options-actions">
				<span class="submit">
					<input name="wolf-theme-options-save" type="submit" class="wolf-theme-options-save button-primary menu-save" value="<?php esc_html_e( 'Save changes', 'sonic' ); ?>">
	...
	?>
		...
		<?php
		$reset_options_confirm = esc_html__( 'Are you sure to want to reset all options ?', 'sonic' );
		?>
		<div id="wolf-options-footer">
			<form method="post" action="<?php echo esc_url( admin_url( 'admin.php?page=wolf-theme-options' ) ); ?>">
				<p id="reset">
					<input name="wolf-reset-all-options" type="submit" value="<?php esc_html_e( 'Reset all options', 'sonic' ); ?>" onclick="if (window.confirm( '<?php echo esc_js( $reset_options_confirm ); ?>' ) )
		...
		public function do_input( $item ) {
			...
			if ( $help ) {
				...
				$help = '<span class="hastip" title="' . esc_attr( esc_html__( 'Click to view the screenshot helper', 'sonic' ) ) . '"><a class="wolf-help-img" href="' . esc_url( WOLF_THEME_URI . '/assets/img/admin/help/' . $item['help'] )  . '"><img src="' . esc_url( WOLF_FRAMEWORK_URI . '/assets/img/help.png' ) . '" alt="help"></a></span>';
		...
		?>
			...
			<tr valign="top" class="<?php echo esc_attr( $class ); ?>"<?php echo sanitize_text_field( $data ); ?>>
				...
				<td class="forminp">
					...
					<br>
					<a href="#" class="button wolf-options-reset-img"><?php esc_html_e( 'Clear', 'sonic' ); ?></a>
					<a href="#" class="button wolf-options-set-img"><?php esc_html_e( 'Choose an image', 'sonic' ); ?></a>
		...
		<?php elseif ( $type == 'file' ) : ?>
			...
			<tr valign="top" class="<?php echo esc_attr( $class ); ?>"<?php echo sanitize_text_field( $data ); ?>>
				...
				<td class="forminp">
					...
					<br><br>
					<a href="#" class="button wolf-options-reset-file"><?php esc_html_e( 'Clear', 'sonic' ); ?></a>
					<a href="#" class="button wolf-options-set-file"><?php esc_html_e( 'Choose a file', 'sonic' ); ?></a>
				</td>
			</tr>
		...
		if ( $font_color ) :
			$options = array(
			 ...
			 ?>
			 	<tr valign="top">
			    		<th scope="row" class="titledesc">
						<label for="<?php echo esc_attr( $prefix ); ?>[<?php echo esc_attr( $field_id ); ?>_font_color]"><?php esc_html_e( 'Font Color', 'sonic' ); ?></label>
					</th>
			...
			<tr valign="top">
		    		<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ); ?>[<?php echo esc_attr( $field_id ); ?>_color]"><?php esc_html_e( 'Background Color', 'sonic' ); ?><br></label>
				</th>
		...
		?>
			<tr valign="top">
		    		<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ); ?>[<?php echo esc_attr( $field_id ); ?>_img]"><?php esc_html_e( 'Background Image', 'sonic' ); ?>
				</label>
				</th>
				...
				<td class="forminp">
					...
					<br><a href="#" class="button wolf-options-reset-bg"><?php esc_html_e( 'Clear', 'sonic' ); ?></a>
					<a href="#" class="button wolf-options-set-bg"><?php esc_html_e( 'Choose an image', 'sonic' ); ?></a>
				</td>
			</tr>
		...
		$options = array( 'no-repeat', 'repeat', 'repeat-x', 'repeat-y' );
		 ?>
		 	<tr valign="top">
		    		<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ); ?>[<?php echo esc_attr( $field_id ); ?>_repeat]"><?php esc_html_e( 'Repeat', 'sonic' ); ?></label>
				</th>
		...
		$options = array(
		 ...
		 ?>
	 		<tr valign="top">
		    		<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ); ?>[<?php echo esc_attr( $field_id ); ?>_position]"><?php esc_html_e( 'Position', 'sonic' ); ?></label>
				</th>
		...
		if ( $do_attachment ) :
			...
			$options = array(
			 ...
			 ?>
		 		<tr valign="top">
			    		<th scope="row" class="titledesc">
						<label for="<?php echo esc_attr( $prefix ); ?>[<?php echo esc_attr( $field_id ); ?>_attachment]"><?php esc_html_e( 'Attachment', 'sonic' ); ?></label>
					</th>
		...
		---------------*/
		$options = array(
			'cover' => esc_html__( 'cover (resize)', 'sonic' ),
			'normal' => esc_html__( 'normal', 'sonic' ),
			'resize' => esc_html__( 'responsive (hard resize)', 'sonic' ),
		);

		?>
			<tr valign="top">
		    		<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ); ?>[<?php echo esc_attr( $field_id ); ?>_size]"><?php esc_html_e( 'Size', 'sonic' ); ?></label>
				</th>
				...
				<?php if ( $parallax ): ?>
					<tr valign="top">
				    		<th scope="row" class="titledesc">
							<label for="<?php echo esc_attr( $prefix ); ?>[<?php echo esc_attr( $field_id ); ?>_parallax]"><?php esc_html_e( 'Parallax', 'sonic' ); ?></label>
							</label>
						</th>
		...
		<div class="<?php echo esc_attr( $class ); ?>"<?php echo sanitize_text_field( $data ); ?>>
			...
			<tr valign="top">
		    		<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_font_name]'; ?>"><?php esc_html_e( 'Font family', 'sonic' ); ?><br>
					</label>
				</th>
			...
			<tr valign="top">
		    		<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_font_color]'; ?>"><?php esc_html_e( 'Font color', 'sonic' ); ?><br>
					</label>
				</th>
			...
			<tr valign="top">
		    		<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_font_weight]'; ?>"><?php esc_html_e( 'Font weight', 'sonic' ); ?><br>
					<small class="description"><?php esc_html_e( 'For example: 400 is normal, 700 is bold.The available font weights depend on the font. Leave empty to use the theme default style', 'sonic' ); ?></small>
					</label>
				</th>
			...
			<tr valign="top">
		    		<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_font_transform]'; ?>"><?php esc_html_e( 'Font transform', 'sonic' ); ?><br>
					</label>
				</th>
			...
			<tr valign="top">
				<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_font_style]'; ?>"><?php esc_html_e( 'Font style', 'sonic' ); ?><br>
					</label>
				</th>
			...
			<tr valign="top">
				<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_font_letter_spacing]'; ?>"><?php esc_html_e( 'Letter Spacing (omit px)', 'sonic' ); ?><br>
					</label>
				</th>
			...
			<table class="form-table">
				...
				<?php if ( ! in_array( 'type', $exclude_params ) ) : ?>
					<tr valign="top" class="option-section-<?php echo esc_attr( $field_id .'_type' ); ?>">
						<th scope="row" class="titledesc">
							<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_type]'; ?>"><?php esc_html_e( 'Video Background type', 'sonic' ); ?><br>
						</th>

						<td class="forminp">
							<select name="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_type]'; ?>">
								<option value="youtube" <?php selected( $type, 'youtube' ); ?>>Youtube</option>
								<option value="selfhosted" <?php selected( $type, 'selfhosted' ); ?>><?php esc_html_e( 'Self hosted', 'sonic' ); ?></option>
							</select>
						</td>
					</tr>
				<?php endif; ?>
				...
				<?php if ( ! in_array( 'youtube_url', $exclude_params ) ) : ?>
					<tr valign="top" class="has-dependency" data-dependency-element="<?php echo esc_attr( $field_id . '_type' ); ?>" data-dependency-values='["youtube"]'>
						<th scope="row" class="titledesc">
							<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_youtube_url]'; ?>"><?php esc_html_e( 'Video Background Youtube URL', 'sonic' ); ?><br>
						</th>
				...
				<?php if ( ! in_array( 'mp4', $exclude_params ) ) : ?>
					<tr valign="top" class="has-dependency" data-dependency-element="<?php echo esc_attr( $field_id . '_type' ); ?>" data-dependency-values='["selfhosted"]'>
						<th scope="row" class="titledesc">
							<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_mp4]'; ?>"><?php esc_html_e( 'mp4 URL', 'sonic' ); ?><br>
						</th>
				...
				<?php if ( ! in_array( 'webm', $exclude_params ) ) : ?>
					<tr valign="top" class="has-dependency" data-dependency-element="<?php echo esc_attr( $field_id . '_type' ); ?>" data-dependency-values='["selfhosted"]'>
						<th scope="row" class="titledesc">
							<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_webm]'; ?>"><?php esc_html_e( 'webm URL', 'sonic' ); ?><br>
						</th>
				...
				<?php if ( ! in_array( 'ogv', $exclude_params ) ) : ?>
					<tr valign="top" class="has-dependency" data-dependency-element="<?php echo esc_attr( $field_id . '_type' ); ?>" data-dependency-values='["selfhosted"]'>
						<th scope="row" class="titledesc">
							<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_ogv]'; ?>"><?php esc_html_e( 'ogv URL', 'sonic' ); ?><br>
						</th>
				...
				<?php if ( ! in_array( 'image', $exclude_params ) ) : ?>
					<tr valign="top">
						<th scope="row" class="titledesc">
							<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_img]'; ?>"><?php esc_html_e( 'Image URL', 'sonic' ); ?><br>
							<small class="description"><?php esc_html_e( 'Image fallback', 'sonic' ); ?></small></label>
						</th>
