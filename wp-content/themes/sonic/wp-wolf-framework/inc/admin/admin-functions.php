...
function wolf_admin_notice( $message = null, $type = null, $cookie_id = null ) {
	...
	if ( $cookie_id ) {
		...
		$dismiss_text = esc_html__( 'Hide permanently', 'sonic' );
...
function wolf_theme_update_notification_message( $link = true ) {
	...
	if ( WOLF_UPDATE_NOTICE ) {
		...
		if ( $changelog && isset( $changelog->latest ) && -1 == version_compare( WOLF_THEME_VERSION, $changelog->latest ) ) {
			$message  = '';
			$message .= '<strong>' . sprintf( esc_html__( 'There is a new version of %s available.', 'sonic' ),  ucfirst( wolf_get_theme_slug() ) ) . '</strong>';
			$message .= ' ' . sprintf( esc_html__( 'You have version %s installed.', 'sonic' ),  WOLF_THEME_VERSION );
			...
			if ( $link ) {
				...
				$message .= sprintf( esc_html__( 'Update to version %s', 'sonic' ),  $changelog->latest );
...
function wolf_help_mark( $id ) {
	return '<span class="hastip" title="' . esc_attr( esc_html__( 'Click to view the screenshot helper', 'sonic' ) ) . '"><a class="wolf-help-img" href="' . esc_url( WOLF_THEME_URI . '/assets/img/admin/help/' . $id )  . '"><img src="' . esc_url( WOLF_FRAMEWORK_URI . '/assets/img/help.png' ) . '" alt="help"></a></span>';
}