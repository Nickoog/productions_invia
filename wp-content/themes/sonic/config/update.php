 ...
 */
function sonic_update_last_posts_shortcodes() {
 ...
 */
function sonic_update_theme_button_attr() {
...
function sonic_set_new_mods() {
	...
	if ( version_compare( $current_version, '1.7.4', '<' ) ) {
		sonic_update_last_posts_shortcodes();
	}

	if ( version_compare( $current_version, '1.8.8', '<' ) ) {
	...
	if ( version_compare( $current_version, '1.9.1', '<' ) ) {
		...
		sonic_update_theme_button_attr();

		set_theme_mod( 'footer_layout', 'boxed' );
	}
}
add_action( 'wolf_do_update', 'sonic_set_new_mods' );