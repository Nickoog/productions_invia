...
$thumb_size = 'sonic-2x2';

if ( 'image' == $format || 'gallery' == $format) {
	$thumb_size = 'sonic-portrait';
}

?>
<article <?php sonic_post_attr(); ?> <?php sonic_article_schema(); ?> <?php post_class( $classes ); ?>  data-bg="<?php echo esc_url( $bg ); ?>">
	<?php wolf_post_start(); ?>
	<section class="entry-frame">
		<?php if ( 'video' == $format ) : ?>
			<?php sonic_get_video_background_from_first_video_url(); ?>
		<?php endif; ?>
		<?php if ( 'gallery' == $format || 'audio' == $format || $is_instagram || $is_tweet ) : ?>
			<?php echo sonic_post_media( true, $thumb_size ); ?>
		<?php endif; ?>
			...
			<div class="table">
				<div class="table-cell">
					...
					<?php if ( 'video' == $format || 'gallery' == $format || 'image' == $format || 'standard' == $format ) : ?>
						...
						<?php
							if ( 'standard' != $format && sonic_get_first_category() ) {
								echo '<span class="entry-frame-category category-label">' . sanitize_text_field( sonic_get_first_category() ) . '</span>';
							}
						?>
						...
						<span class="view-more">
							<?php esc_html_e( 'View post', 'sonic' ); ?>
						</span>
						<!-- </a> -->
					<?php endif; ?>
					</div>
	...
	<?php if ( ! post_password_required() && $standard_post ) : ?>
		<section class="entry-masonry2-content">
				<?php sonic_entry_title(); ?>

			<header class="entry-meta">
				<?php sonic_entry_meta( true, true, true, false ); // display date, author, and category only ?>
			</header><!-- .entry-meta -->
			...
			<footer class="entry-meta">
				<?php sonic_entry_meta( false, false, false, false ); // display author only ?>
				<?php sonic_entry_icon_meta(); ?>
