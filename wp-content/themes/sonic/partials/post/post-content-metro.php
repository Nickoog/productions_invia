...
if ( $metro_thumbnail_size ) {
	...
	if ( $post_id == sonic_get_last_post_id() ) {
...
?>
<?php if ( $bg ) : ?>
	<article <?php sonic_post_attr(); ?> <?php sonic_article_schema(); ?> <?php post_class( $class ); ?>>
		<?php wolf_post_start(); ?>
							...
							<span class="post-metro-entry-meta">
								<?php 
									...
									if ( 'post' == get_post_type( $post_id ) ) {
										...
										echo strip_tags( get_the_category_list( esc_html__( ', ', 'sonic' ) ) );
									
									} elseif( 'work' == get_post_type( $post_id ) ) {
										
										echo strip_tags( get_the_term_list( $post_id, 'work_type', '', esc_html__( ', ', 'sonic' ), '' ) );
									}
								?>
