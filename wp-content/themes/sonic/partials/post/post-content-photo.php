...
$thumb_size = 'sonic-thumb';
$link = sonic_get_first_url();
$permalink = ( 'link' == $format && $link ) ? $link : get_permalink();
?>
<?php if ( has_post_thumbnail() ) : ?>
	<article <?php sonic_post_attr(); ?> <?php post_class( array( 'entry-display-photo' ) ); ?> <?php sonic_article_schema(); ?>>
		<?php wolf_post_start(); ?>
						...
						<div class="post-photo-content table-cell">
							...
							<span class="post-photo-entry-meta">
								<?php 
									...
									printf(
										esc_html__( 'by', 'sonic' ) . ' <span class="author vcard">%s</span>',
										get_the_author()
									);
									echo '</span>';
									...
									printf( esc_html__( ' &mdash; %s ago', 'sonic' ), human_time_diff( get_the_time( 'U' ), current_time( 'timestamp' ) ) );
									echo '</time>';
								?>
