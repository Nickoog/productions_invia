...
$active_class = ( sonic_is_blog_index() ) ? 'active' : '';

if ( is_category() ) {

	// if the category blog display is different, no filter
	if ( wolf_get_theme_mod( 'blog_display', 'standard' ) != sonic_get_blog_display() ) {
		return;
	}
...
<div id="blog-filter-container">
	<ul id="blog-filter">
		<li><a data-url="<?php echo esc_url( sonic_get_blog_url() ); ?>" data-filter="masonry-item" class="<?php echo sanitize_html_class( $active_class ); ?>" href="<?php echo esc_url( sonic_get_blog_url() ); ?>"><?php esc_html_e( 'All', 'sonic' ); ?></a></li>
