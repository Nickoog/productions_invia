...
if ( ! function_exists( 'sonic_shortcode_images_gallery' ) ) {
	/**
	 * Gallery shortcode
	 *
	 ...
	 * @return string
	 */
	function sonic_shortcode_images_gallery( $output = '', $atts, $instance ) {
		...
		$images = explode( ',', $ids );
		$size = ( 'thumbnail' == $size ) ? 'sonic-2x1' : $size;

		if ( ! is_single() && 'gallery' == get_post_format() ) {

			if ( 'masonry2' == sonic_get_blog_display() ) {
				$size = 'sonic-portrait';
			}
			$output = sonic_flexslider_gallery( $images, $size, $orderby );

		} else {

			if ( 'simple' == $layout ) {
				$output = sonic_simple_gallery( $images, $link, $size, $padding, $columns, $hover_effect, $orderby, $inline_style, $class );

			} elseif ( 'mosaic' == $layout ) {
				$carousel = ( $layout == 'carousel_mosaic' ) ? true : false;
				$output = sonic_mosaic_gallery( $images, $link, $hover_effect, $orderby, $carousel, $inline_style, $class );

			
			} elseif ( 'slider' == $layout  ) {
				$output = sonic_flexslider_gallery( $images, $size, $orderby, $inline_style, $class );
			}
		}

		return $output;
	}
	add_filter( 'post_gallery', 'sonic_shortcode_images_gallery', 10, 3 );
}

if ( ! function_exists( 'sonic_flexslider_gallery' ) ) {
	/**
	 ...
	 */
	function sonic_flexslider_gallery( $images = array(), $size = 'sonic-2x1', $orderby, $inline_style = '', $class = '' ) {
		...
		if ( sonic_is_blog() && 'masonry2' == sonic_get_blog_display() ) {
...
if ( ! function_exists( 'sonic_simple_gallery' ) ) {
	/**
	 ...
	 * @return string $output
	 */
	function sonic_simple_gallery( $images = array(), $link = 'file', $size = 'sonic-thumb', $padding = 'no', $columns = 3, $hover_effect = 'default', $orderby = '', $inline_style = '', $class = '' ) {
		...
		foreach ( $images as $image_id ) {
			...
			if ( $attachment ) {
				...
				$file = esc_url( wolf_get_url_from_attachment_id( $image_id, 'sonic-XL' ) );
...
if ( ! function_exists( 'sonic_mosaic_gallery' ) ) {
	/**
	 ...
	 * @return string $output
	 */
	function sonic_mosaic_gallery( $images = array(), $link = 'file', $hover_effect = 'default', $orderby = '', $carousel = true, $inline_style = '', $class = '' ) {
