 ...
 * @return string 
 */
function sonic_post_meta_get_background_css( $selector, $meta_name ) {
	...
	if ( function_exists( 'is_product_category' ) && is_product_category() && get_woocommerce_term_meta( get_queried_object()->term_id, 'thumbnail_id', true ) ) {
		...
		$url = wolf_get_url_from_attachment_id( $thumbnail_id, 'sonic-XL' );
		...
		if ( ! sonic_get_header_post_id() ) {
		...
		} elseif ( sonic_get_header_post_id() ) {

			//debug( sonic_get_header_post_id() );
			...
			if ( $attachment_id ) {
				$url = 'url('. wolf_get_url_from_attachment_id( $attachment_id, 'sonic-XL' ) .')';
			}
 ...
 * Output the post CSS
 */
function sonic_output_post_header_css() {
	...
	if ( 'none' != sonic_get_header_type() ) {
		$css .= "\n";
	    	$css .= sonic_post_meta_get_background_css( 'body.wolf .post-header-container', '_post_bg' );
	}
	wp_add_inline_style( 'sonic-single-post-style', $css );
}
add_action( 'wp_enqueue_scripts', 'sonic_output_post_header_css', 14 );