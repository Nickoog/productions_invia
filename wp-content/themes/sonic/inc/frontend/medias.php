...
if ( ! function_exists( 'sonic_post_class' ) ) {
	/**
	 ...
	 * @return array
	 */
	function sonic_post_class( $classes ) {
		...
		$featured = get_post_meta( $post_id, '_sonic_featured_post', true );
		$has_media = sonic_post_media( false );
		...
		if ( 'standard' == $format || 'image' == $format ) {
			if ( ! preg_match( sonic_get_regex( 'instagram' ), $content ) && ! $has_post_thumbnail ) {
				$classes[] = 'text-only';
			}
		}

		if ( 'audio' == $format && ! sonic_featured_audio( false )  ) {
			$classes[] = 'text-only';
		}

		if ( 'video' == $format && ! sonic_featured_video( false )  ) {
			$classes[] = 'text-only';
		}

		if ( 'gallery' == $format && ! sonic_featured_gallery( false )  ) {
			$classes[] = 'text-only';
		}

		//'audio',
		//'playlist',
		//'sonic_jplayer_playlist',
		//'soundcloud',
		...
		if ( sonic_get_first_soundcloud_url() && 'audio' == $format ) {
			$classes[] = 'is-soundcloud';
		}
			
		if ( sonic_get_first_video_url() || $is_embed_video && 'video' == $format ) {
			$classes[] = 'is-video';
		}

		if ( preg_match( sonic_get_regex( 'twitter' ), $content ) ) {
			$classes[] = 'has-tweet';
		}
			
		if ( 'status' == $format && preg_match( sonic_get_regex( 'twitter' ), $content ) ) {
			$classes[] = 'is-tweet';
		}

		if ( 'image' == $format && preg_match( sonic_get_regex( 'instagram' ), $content ) ) {
		...
		return apply_filters( 'sonic_post_class', $classes );
	}
	add_filter( 'post_class', 'sonic_post_class' );
}

if ( ! function_exists( 'sonic_featured_quote' ) ) {
	/**
...
if ( ! function_exists( 'sonic_featured_instagram' ) ) {
	/**
	 ...
	 * @return string
	 */
	function sonic_featured_instagram( $embed = true ) {


		$has_instagram = preg_match( sonic_get_regex( 'instagram' ), get_the_content(), $match );
...
if ( ! function_exists( 'sonic_featured_tweet' ) ) {
	/**
	 ...
	 * @return string
	 */
	function sonic_featured_tweet( $embed = true ) {

		$has_tweet = preg_match( sonic_get_regex( 'twitter' ), get_the_content(), $match );
...
if ( ! function_exists( 'sonic_featured_gallery' ) ) {
	/**
	 ...
	 * @return string
	 */
	function sonic_featured_gallery( $do_shortcode = true, $size = 'thumbnail' ) {
...
if ( ! function_exists( 'sonic_featured_audio' ) ) {
	/**
	 ...
	 * @return string
	 */
	function sonic_featured_audio( $embed = true ) {

		$audio = null;

		$pattern = get_shortcode_regex();
		$first_url = sonic_get_first_url();
		...
		if ( preg_match( "/$pattern/s", get_the_content(), $match ) ) {
			...
			} elseif ( preg_match( sonic_get_regex( 'soundcloud' ), $first_url ) ) {
...
if ( ! function_exists( 'sonic_featured_video' ) ) {
	/**
	 ...
	 * @return string
	 */
	function sonic_featured_video( $embed = true ) {
		...
		if ( preg_match( "/$pattern/s", get_the_content(), $match ) ) {
			...
			if ( in_array( $match[2], $shortcodes ) ) {
				...
				$first_video_url = sonic_get_first_video_url();
...
if ( ! function_exists( 'sonic_post_media' ) ) {
	/**
	 ...
	 * @return string
	 */
	function sonic_post_media( $embed = true, $thumb_size = 'large' ) {
		...
		$audio = sonic_featured_audio( false );
		$video = sonic_featured_video( false );
		$gallery = sonic_featured_gallery( false );
		$link = sonic_get_first_url();

		$is_standard = 'standard' == $format && $has_thumbnail;
		$is_image = 'image' == $format && $has_thumbnail;
		$is_instagram = ( 'image' == $format ) && preg_match( sonic_get_regex( 'instagram' ), $content );
		...
		$has_tweet = preg_match( sonic_get_regex( 'twitter' ), $content );
		$is_tweet = $has_tweet && 'status' == $format;

		if ( $is_instagram ) {

			$media = sonic_featured_instagram( $embed );

		} elseif ( $is_tweet ) {

			$media = sonic_featured_tweet( $embed );

		} elseif ( $is_video ) {
			
			$media = sonic_featured_video( $embed );

		} elseif ( $is_quote ) {

			$media = sonic_featured_quote();
			...
			$media = sonic_featured_gallery( $embed, $thumb_size );
			...
			if ( $embed ) {
				...
				if ( $has_thumbnail && $is_single_audio ) {
					...
					if ( ! is_single() ) {
						$thumbnail .= '<a href="' . get_permalink() . '" title="' . esc_attr( sprintf( esc_html__( 'Permalink to %s', 'sonic' ), the_title_attribute( 'echo=0' ) ) ) . '" rel="bookmark">';
					}
					
					$thumbnail .= get_the_post_thumbnail( $post_id, 'sonic-2x1' );
				...
				$media .= sonic_featured_audio( $embed );
			...
			$full_img = wolf_get_post_thumbnail_url( 'sonic-XL' );
			...
			if ( ! is_single() ) {
				$media .= '<a class="entry-link" href="' . get_permalink() . '" title="' . esc_attr( sprintf( esc_html__( 'Permalink to %s', 'sonic' ), the_title_attribute( 'echo=0' ) ) ) . '" rel="bookmark">';
			}
...
if ( ! function_exists( 'sonic_get_first_link_url' ) ) {
	/**
	 * Return the URL for the first link in the post content or the permalink if no
	 * URL is found.
	 * Keep this function just in case  'cause we mostly use the function sonic_get_first_url
	 *
	 * @since Sonic 1.0.0
	 * @return string
	 */
	function sonic_get_first_link_url() {
...
if ( ! function_exists( 'sonic_get_first_url' ) ) {
	/**
...
if ( ! function_exists( 'sonic_get_first_soundcloud_url' ) ) {
	/**
	 ...
	 * @return string
	 */
	function sonic_get_first_soundcloud_url() {

		$has_soundlcloud_url = preg_match( sonic_get_regex( 'soundcloud' ), get_the_content(), $match );
...
if ( ! function_exists( 'sonic_get_first_video_url' ) ) {
	/**
	 ...
	 * @return string
	 */
	function sonic_get_first_video_url( $post_id = null ) {
...
if ( ! function_exists( 'sonic_no_video_content' ) ) {
	/**
	 ...
	 * @return
	 */
	function sonic_no_video_content( $post_id = null ) {

		if ( ! $post_id ) {
			$post_id = get_the_ID();
		}

		$no_https = str_replace( 'https', 'http', sonic_get_first_video_url() );
		$with_https = str_replace( 'http', 'https', sonic_get_first_video_url() );
		$content = get_post_field( 'post_content', $post_id );

		$content = str_replace( array( sonic_get_first_video_url(), $no_https, $with_https ), '', $content );

		return apply_filters( 'the_content', $content );
	}
}

if ( ! function_exists( 'sonic_get_regex' ) ) {
	/**
	 ...
	 * @return string
	 */
	function sonic_get_regex( $type = null ) {
...
if ( ! function_exists( 'sonic_override_shortcode_video_dimensions' ) ) {
	/**
	 ...
	 * @return array
	 */
	function sonic_override_shortcode_video_dimensions( $output, $pairs, $atts ) {

		$output['width'] = '800';
		$out['height'] = '450';
		return $output;
	}
	//add_filter( 'shortcode_atts_video', 'sonic_override_shortcode_video_dimensions', 10, 3 );
}

if ( ! function_exists( 'sonic_format_custom_content_output' ) ) {
	/**
	 ...
	 * @return void
	 */
	function sonic_format_custom_content_output( $content ) {
...
if ( ! function_exists( 'sonic_no_media_content' ) ) {
	/**
	 ...
	 * @since Sonic 1.0.0
	 */
	function sonic_no_media_content() {
		...
		if ( ! post_password_required() ) {
			...
			if ( ! is_single() && $post->post_excerpt || is_search() ) {
				...
				$media_https = str_replace( 'http', 'https', sonic_post_media( false ) );
				
				$post_types = array( 'post', 'work', 'video' );

				$content = get_the_content( sonic_more_text() );
				...
				return apply_filters( 'sonic_the_content', $content );
			}
		} else {
			ob_start();
			the_content();
...
if ( ! function_exists( 'sonic_get_video_background_from_first_video_url' ) ) {
	/**
	 ...
	 *
	 */
	function sonic_get_video_background_from_first_video_url() {
		
		if ( sonic_get_first_url() ) {
			
			$video_url = sonic_get_first_url();
			...
			if ( preg_match( '#youtu#', $video_url, $match ) ) {
				...
				echo sonic_youtube_video_bg( $video_url, $img_src );
			
			} elseif ( preg_match( '#.mp4#', $video_url, $match ) ) {

				echo sonic_video_bg( $video_url );
			}
		}
	}
}