 ...
 */
function sonic_enqueue_admin_script() {
	wp_enqueue_script( 'sonic-admin', WOLF_THEME_JS . '/admin.js', array( 'jquery' ), WOLF_THEME_VERSION, true );
	// Add JS global variables
	wp_localize_script(
		'sonic-admin', 'SonicAdminParams', array(
			'subHeadingPlaceholder' => esc_html__( 'Subheading', 'sonic' ),
		)
	);
}
add_action( 'admin_enqueue_scripts', 'sonic_enqueue_admin_script' );
 ...
 * @return array
 */
function sonic_gravatar( $avatar_defaults ) {
	...
	if ( wolf_get_theme_option( 'custom_avatar' ) ) {
		...
		$avatar_defaults[ $custom_avatar_url ] = sprintf( esc_html__( '%s avatar', 'sonic' ), WOLF_THEME_NAME );
	}

	return $avatar_defaults;
}
add_filter( 'avatar_defaults', 'sonic_gravatar' );
 ...
 */
function sonic_sync_font_options_to_theme( $options ) {
...
add_action( 'wpb_after_options_save', 'sonic_sync_font_options_to_theme' );

/**
 * Sync font loader option between theme fonts and WPB fonts
 */
function sonic_sync_font_options_from_theme() {

	if ( class_exists( 'Wolf_Page_Builder' ) ) {
		wpb_update_option( 'fonts', 'google_fonts', wolf_get_theme_option( 'google_fonts' ) );
	}
}
add_action( 'wolf_after_options_save', 'sonic_sync_font_options_from_theme' );

/**
 * Remove unuseful post formats on work posts
 */
function sonic_work_post_formats() {
...
add_action( 'load-post.php', 'sonic_work_post_formats' );
add_action( 'load-post-new.php', 'sonic_work_post_formats' );
 ...
 */
function sonic_admin_custom_css() {
...
add_action( 'admin_enqueue_scripts', 'sonic_admin_custom_css' );
 ...
 * @return $mods
 */
function sonic_set_default_customizer_images( $mods ) {
...
add_filter( 'wolf_default_mods', 'sonic_set_default_customizer_images' );

/**
 * Define WooCommerce image sizes on theme activation
 *
 * Can be overwritten with the sonic_woocommerce_thumbnail_sizes filter
 */
function sonic_woocommerce_image_sizes() {
	global $pagenow;
	...
	$woocommerce_thumbnails = apply_filters( 'sonic_woocommerce_thumbnail_sizes', array(
			'catalog' => array(
				'width' 	=> '400',	// px
				'height'	=> '400',	// px
				'crop'	=> 1 		// true
...
add_action( 'after_switch_theme', 'sonic_woocommerce_image_sizes', 1 );
 ...
 */
function sonic_set_wbounce_content() {

	// wBounce
	$wbounce_content = '<div class="modal-title">
	<h3>' . esc_html__( 'Never miss out any news!', 'sonic' ) . '</h3>
</div>

<div class="modal-body">
	<p>' . esc_html__( 'Subscribe to receive the latest news by email', 'sonic' ) . '</p>
	[wpb_mailchimp use_bg="false" label=""]
</div>

<div class="modal-footer">
       	<p>' . esc_html__( 'no thanks', 'sonic' ) . '</p>
</div>';

	update_option( 'wbounce_content', apply_filters( 'sonic_wbounce_content', $wbounce_content ) );

}
add_action( 'after_switch_theme', 'sonic_set_wbounce_content', 1 );
 ...
 * These are the most common options and can filtered using the sonic_default_options filter
 *
 * @see wp-wolf-framework/admin/class-admin.php
 * @see wp-wolf-framework/admin/class-options.php
 */
function sonic_default_theme_options_init() {
	...
	$default_options['share'] = esc_html__( 'Share', 'sonic' );
	...
	$default_options = apply_filters( 'sonic_default_options', $default_options );
...
add_action( 'wolf_theme_default_options_init', 'sonic_default_theme_options_init' );
 ...
 * @see wp-wolf-framework/admin/class-admin.php
 */
function sonic_default_wp_options_init() {
	
	// default WP options
	update_option( 'image_default_link_type', 'file' );
}
add_action( 'wolf_wp_default_options_init', 'sonic_default_wp_options_init' );
 ...
 */
function sonic_set_wpb_default_google_fonts( $settings ) {
...
add_filter( 'wpb_default_settings', 'sonic_set_wpb_default_google_fonts' );


/**
 * Diable OCDI PT branding
 *
 * @param bool
 * @return bool
 */
function sonic_disable_ocdi_pt_branding( $bool ) {
	return true;
}
add_filter( 'pt-ocdi/disable_pt_branding', 'sonic_disable_ocdi_pt_branding' );
 ...
 */
function sonic_set_menu_locations( $menus = array() ) {
 ...
 */
function sonic_set_permalinks_after_import() {
...
add_action( 'pt-ocdi/after_import', 'sonic_set_permalinks_after_import' );
 ...
 */
function sonic_set_pages_after_import() {
...
add_action( 'pt-ocdi/after_import', 'sonic_set_pages_after_import' );
 ...
 * @return string
 */
function sonic_plugin_intro_text( $default_text ) {
	
	ob_start();
	?>
	<div class="wolf-ocdi-intro-text">

		<h1><?php esc_html_e( 'Install demo content', 'sonic' ); ?></h1>

		<p class="about-description">
			<?php esc_html_e( 'Importing demo data is the easiest way to setup your theme. It will allow you to quickly edit everything instead of creating content from scratch.', 'sonic' ); ?>
		</p>

		<hr>

		<p><?php esc_html_e( 'When you import the data, the following things might happen', 'sonic' ); ?>:</p>

		<ul>
			<li><?php esc_html_e( 'No existing posts, pages, categories, images, custom post types or any other data will be deleted or modified.', 'sonic' ); ?></li>
			<li><?php esc_html_e( 'Posts, pages, images, widgets, menus and other theme settings will get imported.', 'sonic' ); ?></li>
			<li><?php esc_html_e( 'Images will be downloaded from our server.', 'sonic' ); ?></li>
			<li><?php esc_html_e( 'Please click on the "Import" button only once and wait, it can take a few minutes.', 'sonic' ); ?></li>
		</ul>

		<div class="wolf-ocdi-notice">
			<h4><?php esc_html_e( 'Important', 'sonic' ); ?></h4>

			<ul>
				<li>
					<?php
					printf(
						__( 'Before you begin, make sure that your server settings fulfill the <a href="%s">server requirements</a>.', 'sonic' ),
						admin_url( 'themes.php?page=wolf-theme-about' )
					);
				?>
				</li>
				<li><?php printf( esc_html__( 'Make sure all the required plugins are activated.', 'sonic' ), '' ); ?></li>
				<li><?php esc_html_e( 'Deactivate all 3rd party plugins except the one recommended by the theme.', 'sonic' ); ?></li>
				<li><?php esc_html_e( 'It is recommended to delete all pages that may have been created by plugins and empty the trash to avoid duplicate content.', 'sonic' ); ?></li>
				<li><?php esc_html_e( 'It is always recommended to run the import on a fresh WordPress installation.', 'sonic' ); ?></li>
				<li><?php esc_html_e( 'Some of the images may be replaced by placeholder images if they are copyrighted material.', 'sonic' ); ?></li>
				<li><?php esc_html_e( 'As custom scripts may be used on the demo website to showcase layout variation options, some layout example pages may show the default customizer settings.', 'sonic' ); ?></li>
				<li><?php
					printf(
						__( 'In the case of import failure, we recommend resetting your install before try it again using <a href="%s" target="_blank">WordPress Reset</a> plugin.', 'sonic' ),
...
add_filter( 'pt-ocdi/plugin_intro_text', 'sonic_plugin_intro_text' );
 ...
 */
function sonic_replace_wpb_content_urls_after_import() {
...
add_action( 'pt-ocdi/after_import', 'sonic_replace_wpb_content_urls_after_import' );

/**
 * Replace hard coded URLs from demo data to local URL
 */
function sonic_replace_menu_item_custom_urls_after_import() {
...
add_action( 'pt-ocdi/after_import', 'sonic_replace_menu_item_custom_urls_after_import' );