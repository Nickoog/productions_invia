<?php
/**
 * Sonic Menu item custom fields example
 *
 ...
 * @since 0.1.0
 */
class Sonic_Menu_Item_Custom_Fields {
		...
		?>
			<p class="field-_mega-menu description description-wide">
				<label for="edit-_mega-menu-<?php echo esc_attr( $item_id ) ?>">
					...
					<?php esc_html_e( 'Mega Menu (only available for first level items)', 'sonic' ) ?>
			...
			<p class="field-_menu-item-not-linked description description-wide">
				<label for="edit-_menu-item-not-linked-<?php echo esc_attr( $item_id ) ?>">
					...
					<?php esc_html_e( 'Mega Menu 2nd level or dropdown item', 'sonic' ) ?>
			...
			<p class="field-_menu-item-hidden description description-wide">
				<label for="edit-_menu-item-hidden-<?php echo esc_attr( $item_id ) ?>">
					...
					<?php esc_html_e( 'Hide item on mega menu (for mega menu 2nd level only)', 'sonic' ) ?>
			...
			<p class="field-_menu-item-button-style description description-wide">
				<label for="edit-_menu-item-button-style-<?php echo esc_attr( $item_id ) ?>">
					...
					<?php esc_html_e( 'Button Style (only available for first level items)', 'sonic' ) ?>
			...
			<p class="field-_menu-item-scroll description description-wide">
				<label for="edit-_menu-item-scroll-<?php echo esc_attr( $item_id ) ?>">
					...
					<?php esc_html_e( 'Scroll to an anchor', 'sonic' ) ?>
				</label>
			</p>
			<p class="field-_mega-menu-cols description description-wide">
					<label for="edit-_mega-menu-cols-<?php echo esc_attr( $item_id ) ?>"><?php esc_html_e( 'Mega Menu Columns', 'sonic' ) ?></label>
						<br>
			...
			<p class="field-_mega-menu-tagline description description-wide">
				...
				<?php esc_html_e( 'Optional Mega Menu Tagline (only available for first level items)', 'sonic' ) ?>
			</p>
			<p class="field-background description description-wide">
				...
				<label for="edit-_menu-item-background-<?php echo esc_attr( $item_id ) ?>">
					<?php esc_html_e( 'Background Image (only for 1rst level mega menu)', 'sonic' ) ?>
			...
			<p class="field-_menu-item-background-repeat description description-wide">
				<label for="edit-_menu-item-background-repeat-<?php echo esc_attr( $item_id ) ?>"><?php esc_html_e( 'Background repeat', 'sonic' ) ?></label><br>
					<select name="_menu-item-background-repeat[<?php echo esc_attr( $item_id ); ?>]">
						<option value="no-repeat" <?php selected( get_post_meta( $item_id, '_menu-item-background-repeat', true ), 'no-repeat' ); ?>><?php esc_html_e( 'no repeat', 'sonic' ); ?></option>
						<option value="repeat" <?php selected( get_post_meta( $item_id, '_menu-item-background-repeat', true ), 'repeat' ); ?>><?php esc_html_e( 'repeat', 'sonic' ); ?></option>
					</select>
			</p>
			<?php if ( class_exists( 'Wolf_Page_Builder' ) ) : ?>
				...
				<p class="field-custom description description-wide wolf-searchable-container">
					<label for="edit-_menu-item-icon-<?php echo esc_attr( $item_id ) ?>"><?php esc_html_e( 'Icon', 'sonic' ) ?></label><br />
						<span><?php printf(
						...
						echo '<option value="">' . esc_html__( 'None', 'sonic' ) . '</option>';
				...
				<p class="field-_menu-item-icon-position description description-wide">
					<label for="edit-_menu-item-icon-position-<?php echo esc_attr( $item_id ) ?>"><?php esc_html_e( 'Icon position', 'sonic' ) ?></label>
						<br>
						<select name="_menu-item-icon-position[<?php echo esc_attr( $item_id ); ?>]">
							<option value="before" <?php selected( get_post_meta( $item_id, '_menu-item-icon-position', true ), 'before' ); ?>><?php esc_html_e( 'before', 'sonic' ); ?></option>
							<option value="after" <?php selected( get_post_meta( $item_id, '_menu-item-icon-position', true ), 'after' ); ?>><?php esc_html_e( 'after', 'sonic' ); ?></option>
						</select>
				</p>
			<?php endif; ?>
		<?php
	...
	public static function _columns( $columns ) {
		//$columns['_menu-item-icon'] = esc_html__( 'Icon', 'sonic' );
		//$columns['_mega-menu'] = esc_html__( 'Mega Menu', 'sonic' );
		//$columns['_menu-item-not-linked'] = esc_html__( 'Sub Menu title type', 'sonic' );
		//$columns['_menu-item-hidden'] = esc_html__( 'Sub Menu title type', 'sonic' );

		return $columns;
	}
}
Sonic_Menu_Item_Custom_Fields::init();
