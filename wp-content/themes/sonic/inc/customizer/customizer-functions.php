 ...
 * @return string
 */
function sonic_get_color_scheme_option() {
	return apply_filters( 'sonic_color_scheme_option', get_theme_mod( 'color_scheme', get_theme_mod( 'skin', 'default' ) ) );
}

/**
 * Registers color schemes for Sonic.
 *
 * Can be filtered with {@see 'sonic_color_schemes'}.
 *
 ...
 */
function sonic_get_color_schemes() {
	/**
	 * Filter the color schemes registered for use with Sonic.
	 *
	 ...
	 * }
	 */
	return apply_filters( 'sonic_color_schemes', array(
		'default' => array(
			'label'  => esc_html__( 'Default', 'sonic' ),
			'colors' => array(
				'#282828', // bg
				'#ffffff', // page bg
				'#007acc', // accent
 ...
 */
function sonic_customize_deregister( $wp_customize ) {
	$wp_customize->remove_control( 'background_color' );
	$wp_customize->remove_control( 'header_textcolor' );
}
add_action( 'customize_register', 'sonic_customize_deregister', 11 );
...
if ( ! function_exists( 'sonic_get_color_scheme' ) ) {
	/**
	 * Retrieves the current Sonic color scheme.
	 *
	 * Create your own sonic_get_color_scheme() function to override in a child theme.
	 *
	 * @return array An associative array of either the current or default color scheme HEX values.
	 */
	function sonic_get_color_scheme() {
		$color_scheme_option = sonic_get_color_scheme_option();
		$color_schemes = sonic_get_color_schemes();
...
if ( ! function_exists( 'sonic_get_color_scheme_choices' ) ) {
	/**
	 * Retrieves an array of color scheme choices registered for Sonic.
	 *
	 * Create your own sonic_get_color_scheme_choices() function to override
	 * in a child theme.
	 *
	 *
	 * @return array Array of color schemes.
	 */
	function sonic_get_color_scheme_choices() {
		$color_schemes = sonic_get_color_schemes();
...
if ( ! function_exists( 'sonic_sanitize_color_scheme' ) ) {
	/**
	 * Handles sanitization for Sonic color schemes.
	 *
	 * Create your own sonic_sanitize_color_scheme() function to override
	 * in a child theme.
	 *
	 * @param string $value Color scheme name value.
	 * @return string Color scheme name.
	 */
	function sonic_sanitize_color_scheme( $value ) {
		$color_schemes = sonic_get_color_scheme_choices();
 ...
 *
 */
function sonic_customize_control_js() {
	
	wp_enqueue_style( 'sonic-customizer-style', WOLF_THEME_CSS . '/customizer.css' );

	$version = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? time() : WOLF_THEME_VERSION;
	wp_enqueue_script( 'sonic-customize-color-scheme-control', WOLF_THEME_JS . '/color-scheme-control.js', array( 'customize-controls', 'iris', 'underscore', 'wp-util' ), $version, true );
	wp_localize_script( 'sonic-customize-color-scheme-control', 'colorScheme', sonic_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'sonic_customize_control_js' );

/**
 * Enqueue customizer preview script
 *
 */
function sonic_customize_preview_js() {
	
	$version = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? time() : WOLF_THEME_VERSION;
	wp_enqueue_script( 'sonic-customize-preview', WOLF_THEME_JS . '/customize-preview.js', array( 'jquery', 'customize-preview' ), $version, true );
}
add_action( 'customize_preview_init', 'sonic_customize_preview_js' );
 ...
 */
function sonic_get_color_scheme_css( $colors ) {
	...
	.wolf-2016 a.wolf_add_to_wishlist:hover,
		.gaia a.wolf_add_to_wishlist:hover,
		.wolf-2016 a.add_to_cart_button:hover,
		.gaia a.add_to_cart_button:hover,
		.wolf-2016 a.more-link:hover,
 ...
 * @return array $colors
 */
function sonic_get_template_identifiers() {
	$colors = array(
		'body_background_color',
		'page_background_color',
		'accent_color',
 ...
 * @return array $colors
 */
function sonic_get_template_colors() {

	$colors = array();

	foreach ( sonic_get_template_identifiers() as $id ) {
		$colors[ $id ] =  '{{ data.' . $id . ' }}';
	}
 ...
 * Customizer preview.
 */
function sonic_color_scheme_css_template() {
	
	$colors = sonic_get_template_colors();
	?>
	<script type="text/html" id="tmpl-sonic-color-scheme">
		<?php echo sonic_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'sonic_color_scheme_css_template' );

/**
 * Get bg CSS
 *
 */
function sonic_get_customizer_bg_css( $selectors = array() ) {
