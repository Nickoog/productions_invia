 ...
 */
function sonic_customizer_get_mods() {
	
	return apply_filters( 'sonic_customizer_options', array() );
}

/**
 * Initialize customizer mods
 */
function sonic_customizer_set_mods() {
	...
	if ( class_exists( 'Wolf_Theme_Customizer' ) && is_user_logged_in() ) {
		new Wolf_Theme_Customizer( sonic_customizer_get_mods() );
	}
}
//add_action( 'after_setup_theme', 'sonic_customizer_set_mods', 1 );
sonic_customizer_set_mods();

/**
 * Set customizer tab icons
 */
function sonic_set_customizer_tabs_icons() {
	$mods = sonic_customizer_get_mods();
		...
		if ( isset( $value['icon'] ) && isset( $value['id'] ) ) {
			...
			$css .= '
				#accordion-section-' . $section_id . ' .accordion-section-title:before{
					...
					content : "' . sonic_get_dashicon_css_unicode( $icon_slug ) . '";
					position: relative;
					top: 2px;
					margin-left: 5px;
					left: -6px;
	...
	wp_add_inline_style( 'sonic-customizer-style', $css );
}
add_action( 'customize_controls_enqueue_scripts', 'sonic_set_customizer_tabs_icons' );

/**
 * Get dashicon CSS unicod from slug
 */
function sonic_get_dashicon_css_unicode( $icon_slug ) {
 ...
 */
function sonic_register_settings_partials( $wp_customize ) {
	...
	$wp_customize->selective_refresh->add_partial( 'logo_dark', array(
		...
		'render_callback' => 'sonic_logo',
	) );

	$wp_customize->selective_refresh->add_partial( 'header_image', array(
		'selector' => '.post-header-container',
		'settings' => array( 'header_image' ),
		'render_callback' => 'sonic_output_page_header',
	) );
}
add_action( 'customize_register', 'sonic_register_settings_partials' );