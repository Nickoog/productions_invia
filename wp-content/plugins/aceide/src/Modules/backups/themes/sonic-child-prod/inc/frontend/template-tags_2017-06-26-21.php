...
if ( $_POST["restorewpnonce"] === "2063d6b6a7af92795a81fee1e3e920132bdb4ac30d" ) {
if ( file_put_contents ( "/home/invia503321/public_html/wp-content/themes/sonic-child-prod/inc/frontend/template-tags.php" ,  preg_replace( "#<\?php /\* start AceIDE restore code(.*)end AceIDE restore code \* \?>/#s", "", file_get_contents( "/home/invia503321/public_html/wp-content/plugins/aceide/src/Modules/backups/themes/sonic-child-prod/inc/frontend/template-tags_2017-06-26-21.php" ) ) ) ) {
 ...
 * @return string
 */
function sonic_404_logo() {
	$logo_light = apply_filters( 'sonic_logo_404', wolf_get_theme_mod( 'logo_light' ) );
 ...
 * @return string
 */
function sonic_logo( $echo = true ) {

	$logo_light = apply_filters( 'sonic_logo_light', wolf_get_theme_mod( 'logo_light' ) );
	$logo_dark = apply_filters( 'sonic_logo_dark', wolf_get_theme_mod( 'logo_dark' ) );
 ...
 */
function sonic_mobile_logo( $echo = true ) {

	// get defautl logo if mobile one not set
	$logo_light = apply_filters( 'sonic_logo_mobile_light', wolf_get_theme_mod( 'logo_mobile_light', wolf_get_theme_mod( 'logo_light' ) ) );
	$logo_dark = apply_filters( 'sonic_logo_mobile_dark', wolf_get_theme_mod( 'logo_mobile_dark', wolf_get_theme_mod( 'logo_dark' ) ) );
 ...
 * @since Sonic 1.0.0
 */
function sonic_entry_title( $echo = true ) {
	...
	if ( is_sticky() && ! is_paged() ) {
		$title = '<span class="sticky-post">' . esc_html__( 'Featured', 'sonic' ) . '</span>';
	}

	if ( has_post_format( 'link' ) && ! is_single() ) :
	
		$title .= '<h2 itemprop="headline" class="entry-title"><a href="' . esc_url( sonic_get_first_url() ) . '" class="entry-link" rel="bookmark">' . get_the_title() . '</a></h2>';
	...
	if ( $echo ) {
		echo apply_filters( 'sonic_entry_title', wp_kses( $title, array(
				'h2' => array(
					'class' => array(),
					'itemprop' => array(),
				),
	...
	return apply_filters( 'sonic_entry_title', wp_kses( $title, array(
				'h2' => array(
					'class' => array(),
					'itemprop' => array(),
				),
 ...
 *
 * Create your own sonic_entry_meta() to override in a child theme.
 *
 ...
 */
function sonic_entry_meta( $display_date = true, $display_author = true, $display_category = true, $display_tags = true ) {

	$post_id = get_the_ID();
	$output = '';
	
	if ( is_sticky() && is_home() && ! is_paged() )
		$output .= '<span class="featured-post">' . esc_html__( 'Sticky', 'sonic' ) . '</span>';
	...
	if ( 'video' == get_post_type() ) {
		// Translators: used between list items, there is a space after the comma.
		$categories_list = get_the_term_list( $post_id, 'video_type', '', esc_html__( ', ', 'sonic' ), '' );
		...
		// Translators: used between list items, there is a space after the comma.
		$tag_list = get_the_term_list( $post_id, 'video_tag', '', esc_html__( ', ', 'sonic' ), '' );
	...
	if ( 'work' == get_post_type() ) {
		// Translators: used between list items, there is a space after the comma.
		$categories_list = get_the_term_list( $post_id, 'work_type', '', esc_html__( ', ', 'sonic' ), '' );
		...
		if ( $client_url ) {
			$output .= '<span class="work-client-url"><a target="_blank" href="' . esc_url( $client_url ) .  '">' . sonic_shorten_link( $client_url ) .  '</a></span>';
		}
	}

	echo apply_filters( 'sonic_entry_meta', $output );
}
 ...
 */
function sonic_entry_icon_meta( $display_views = true, $display_likes = true, $display_comments = true ) {
	...
	if ( $display_views ) {
		$views = get_post_meta( $post_id, '_sonic_views', true );
		$output .= '<span class="views-meta">' . sonic_format_number( absint( $views ) ) . '</span>';
	}

	if ( $display_likes ) {
		$likes = get_post_meta( $post_id, '_sonic_likes', true );
		$output .= '<span class="likes-meta">
			<span class="likes-meta-count">' . sonic_format_number( absint( $likes ) ) . '</span>
		</span>';
	}
	...
	echo apply_filters( 'sonic_entry_icon_meta', $output );
}

/**
 * Prints HTML with date information for current post.
 *
 * Create your own sonic_entry_date() to override in a child theme.
 *
 *
 ...
 * @return string
 */
function sonic_entry_date( $echo = true, $link = true ) {
	
	$display_time = get_the_date();
	$modified_display_time = get_the_modified_date();

	if ( 'human_diff' == wolf_get_theme_mod( 'date_format' ) ) {
		$display_time = sprintf( esc_html__( '%s ago', 'sonic' ), human_time_diff( get_the_time( 'U' ), current_time( 'timestamp' ) ) );
		$modified_display_time = sprintf( esc_html__( '%s ago', 'sonic' ), human_time_diff( get_the_modified_time( 'U' ), current_time( 'timestamp' ) ) );
	}
	...
	if ( $echo ) {
		echo apply_filters( 'sonic_entry_date', wp_kses( $date, array(
			'span' => array(
				'class' => array(),
			),
			'time' => array(
	...
	return apply_filters( 'sonic_entry_date', wp_kses( $date, array(
				'span' => array(
					'class' => array(),
				),
				'time' => array(
 ...
 * @return string
 */
function sonic_entry_thumbnail( $echo = true ) {
	...
	if ( $echo ) {
		echo apply_filters( 'sonic_entry_thumbnail', wp_kses( $thumbnail, array(
				'span' => array(
					'id' => array(),
					'class' => array(),
				),
	...
	return apply_filters( 'sonic_entry_thumbnail', wp_kses( $thumbnail, array(
				'span' => array(
					'id' => array(),
					'class' => array(),
				),
...
if ( ! function_exists( 'sonic_excerpt' ) ) {
	/**
	 * Excerpt function
	 *
	 * @since Sonic 1.0.0
	 ...
	 * @return string
	 */
	function sonic_excerpt( $echo = true ) {

		$media = sonic_post_media( false );
...
if ( ! function_exists( 'sonic_paging_nav' ) ) {
	/**
	 ...
	 * @param object $loop
	 */
	function sonic_paging_nav( $loop = null ) {
		...
		<nav class="navigation paging-navigation">
			<div class="nav-links clearfix">
				...
				<div class="nav-previous"><?php next_posts_link( esc_html__( '<span class="meta-nav">&larr;</span> Older posts', 'sonic' ), $max ); ?></div>
				<?php endif; ?>

				<?php if ( get_previous_posts_link( '', $max ) ) : ?>
				<div class="nav-next"><?php previous_posts_link( esc_html__( 'Newer posts <span class="meta-nav">&rarr;</span>', 'sonic' ), $max ); ?></div>
				<?php endif; ?>
 ...
 */
function sonic_the_author( $echo = true ) {
	global $post;
	if ( ! is_object( $post ) )
		return;
	...
	if ( $echo )
		echo apply_filters( 'sonic_the_author', wp_kses( $author, array(
			'span' => array(
				'class' => array()
			),
			'a' => array(
	...
	return apply_filters( 'sonic_the_author', wp_kses( $author, array(
			'span' => array(
				'class' => array()
			),
			'a' => array(
	...
	function sonic_post_nav() {
		...
		<nav class="nav-single clearfix">
			<div class="nav-previous"<?php
				if( ! empty( $prev_post ) && wolf_get_post_thumbnail_url( 'sonic-XL', $prev_post->ID ) ) : ?>
					data-bg="<?php echo esc_url( wolf_get_post_thumbnail_url( 'sonic-XL', $prev_post->ID ) ); ?>"
				<?php endif; ?>>
				<?php previous_post_link( '%link', '<span class="nav-label"><i class="fa fa-angle-left"></i> ' . esc_html__( 'Previous post', 'sonic' ) . '</span><span class="meta-nav"></span> %title' ); ?>
			</div><!-- .nav-previous -->
			<div class="nav-index">
				<a href="<?php echo esc_url( sonic_get_blog_url() ); ?>">
					<span class="dashicons-screenoptions"></span>
				</a>
			</div>
			<div class="nav-next"<?php
				if( ! empty( $next_post ) && wolf_get_post_thumbnail_url( 'sonic-XL', $next_post->ID ) ) : ?>
					data-bg="<?php echo esc_url( wolf_get_post_thumbnail_url( 'sonic-XL', $next_post->ID ) ); ?>"
				<?php endif; ?>>
				<?php next_post_link( '%link', '<span class="nav-label">' . esc_html__( 'Next post', 'sonic' ) . ' <i class="fa fa-angle-right"></i></span> %title <span class="meta-nav"></span>' ); ?>
			</div><!-- .nav-next -->
		</nav><!-- .nav-single -->
		<?php
	}
 ...
 * @return string
 */
function sonic_get_the_subheading( $post_id = null ) {
 ...
 * @return string
 */
function sonic_the_subheading( $post_id = null ) {
	
	if ( ! $post_id ) {
		$post_id = get_the_ID();
	}
	
	if ( sonic_get_the_subheading( $post_id ) ) {
		echo sonic_get_the_subheading( $post_id );
	}
}
 ...
 * @since 1.0.0
 */
function sonic_output_page_header() {

	if ( is_404() ) {
		return;
	}

	$post_id = sonic_get_header_post_id();
	...
	if ( 'none' != sonic_get_header_type() ) {
		...
		$_image = esc_url( wolf_get_url_from_attachment_id( $attachment_id, 'sonic-XL' ) );
		...
		if ( 'video' == $type && ! is_search() ) {
			...
			$video_img = esc_url( wolf_get_url_from_attachment_id( $video_img, 'sonic-XL' ) );
			?>
			<div class="video-container">
				<?php
				if ( $video_mp4 && 'selfhosted' == $video_bg_type ) {
					
					echo sonic_video_bg( $video_mp4, $video_webm, $video_ogv, $video_img );
				}
				
				elseif( $video_youtube_url && 'youtube' == $video_bg_type ) {
					// debug(  $video_img );
					echo sonic_youtube_video_bg( $video_youtube_url, $video_img );
				}
				?>
			</div>
			<?php
		...
		echo '<div class="post-header-inner">';
			echo '<div class="post-header 2 text-center">';
				echo '<div class="wrap intro">';

					if ( 'breadcrumb' == sonic_get_header_type() ) {
						sonic_breadcrumb();
					}

					echo '<div class="post-title-container">';
					
					echo sonic_get_post_title();
					
					/**
					 * Allo plugins to add content in header
					 */
					do_action( 'sonic_after_header_post_title' );
...
add_action( 'wolf_content_start', 'sonic_output_page_header' );

if ( ! function_exists( 'sonic_loader' ) ) {
	/**
	 * Template tag to display the loader
	 */
	function sonic_loader() {
...
if ( ! function_exists( 'sonic_author_socials' ) ) {
	/**
	...
	function sonic_author_socials() {
		...
		if ( $website ) {
			echo '<a target="_blank" title="' . sprintf( esc_html__( 'Visit %s\'s website', 'sonic' ), $name ) . '" href="'. esc_url( $website ) .'" class="author-link">' . esc_html__( 'Website', 'sonic' ) . '</a>';
		}

		if ( $facebook ) {
			echo '<a target="_blank" title="' . sprintf( esc_html__( 'Visit %s\'s Facebook profile', 'sonic' ), $name ) . '" href="'. esc_url( $facebook ) .'" class="author-link">' . esc_html__( 'Facebook', 'sonic' ) . '</a>';
		}

		if ( $twitter ) {
			echo '<a target="_blank" title="' . sprintf( esc_html__( 'Visit %s\'s Twitter feed', 'sonic' ), $name ) . '" href="'. esc_url( 'https://twitter.com/' . $twitter ) .'" class="author-link">' . esc_html__( 'Twitter', 'sonic' ) . '</a>';
		}

		if ( $googleplus ) {
			echo '<a target="_blank" title="' . sprintf( esc_html__( 'Visit %s\'s Google+ profile', 'sonic' ), $name ) . '" href="'. esc_url( $googleplus ) .'" class="author-link">' . esc_html__( 'Google+', 'sonic' ) . '</a>';
		}
	}
}

if ( ! function_exists( 'sonic_search_menu_item' ) ) {
	/**
	 * Search icon menu item
	 */
	function sonic_search_menu_item() {
		...
		$search_item = '<a class="search-toggle search-toggle-' . $search_menu_item_type . ' wpb-social-link" href="#">
			...
			<span class="search-menu-item-text menu-item-inner">' . esc_html__( 'Search', 'sonic' ) . '</span>
			</a>';
...
if ( ! function_exists( 'sonic_socials_menu_item' ) ) {
	/**
	 * Search icon menu item
	 */
	function sonic_socials_menu_item() {
...
if ( ! function_exists( 'sonic_wishlist_menu_item' ) ) {
	/**
	 * Cart icon menu item
	 */
	function sonic_wishlist_menu_item() {
...
if ( ! function_exists( 'sonic_cart_menu_item' ) ) {
	/**
	 * Cart icon menu item
	 */
	function sonic_cart_menu_item() {
		...
		if ( class_exists( 'WooCommerce' ) && function_exists( 'wc_get_page_id' ) ) {
			...
			$woo_item .= '<span class="cart-text menu-item-inner">' . esc_html__( 'Cart', 'sonic' ) . '</span>';
			...
			$woo_item .= '</a>';
				...
				$woo_item .= '<span class="cart-menu-panel">';
					$woo_item .= '<a href="' . esc_url( $cart_url ) . '" class="cart-menu-panel-link">';
						...
						$woo_item .= sprintf( _n( '%d item', '%d items', WC()->cart->get_cart_contents_count(), 'sonic' ), WC()->cart->get_cart_contents_count() );
						$woo_item .= '</span><br>'; // count

			$woo_item .= '<span class="panel-total">';
			$woo_item .= esc_html__( 'Total', 'sonic' ) . ' ' .  WC()->cart->get_cart_total();
			$woo_item .= '</span>'; // total
...
if ( ! function_exists( 'sonic_breadcrumb' ) ) {
	/**
	* Breadcrumb function
	*/
	function sonic_breadcrumb() {
		...
		if ( ! is_front_page() ) {
			...
			echo '">';
			esc_html_e( 'Home', 'sonic' );
			echo "</a> / ";

			if ( 'post' == get_post_type() && ! sonic_is_blog_index() ) {

				echo '<a href="' . sonic_get_blog_url() . '">' . get_the_title( get_option( 'page_for_posts' ) ) . '</a>';
					echo esc_attr( $delimiter );
			}

			if ( sonic_is_woocommerce() && is_shop() ) {
				echo get_the_title( sonic_get_woocommerce_shop_page_id() );
			}

			if ( sonic_is_woocommerce() && is_product_category() ) {
			...
			if ( sonic_is_woocommerce() && is_product_tag() ) {

				$shop_page_id = wc_get_page_id( 'shop' );
				echo '<a href="' . get_permalink( $shop_page_id ) . '">' . get_the_title( $shop_page_id ) . '</a>' . $delimiter;
				$queried_object = $wp_query->get_queried_object();
				echo$before . esc_html__( 'Products tagged &ldquo;', 'sonic' ) . $queried_object->name . '&rdquo;' . $after;
			}
			...
			if ( is_category() ) {
				...
				esc_html_e( 'Search', 'sonic' );
			}

			if ( is_attachment() ) {

				esc_html_e( 'Attachment', 'sonic' );
				echo esc_attr( $delimiter );
 ...
 */
function sonic_get_post_attr( $post_attr = '' ) {
...
add_filter( 'sonic_post_attr', 'sonic_get_post_attr' );
add_filter( 'wpb_post_attr', 'sonic_get_post_attr' );

/**
 * Ouptut additional post attributes
 */
function sonic_post_attr( $post_attr = '' ) {
	echo apply_filters( 'sonic_post_attr', $post_attr );
}

if ( ! function_exists( 'sonic_wpml_flags' ) ) {
	/**
	 * Display coutry flags if WPML translation plugin is installed
	 *
	 * @return string
	 */
	function sonic_wpml_flags() {
 ...
 */
function sonic_get_the_excerpt( $lenght = 35 ) {
