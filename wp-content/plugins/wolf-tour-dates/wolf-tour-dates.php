 ...
 * Plugin URI: http://wolfthemes.com/plugin/wolf-tour-dates
...
if ( ! class_exists( 'Wolf_Tour_Dates' ) ) {
	...
	class Wolf_Tour_Dates {
		...
		public function __construct() {
			...
			add_shortcode( 'wolf_upcoming_shows_widget', array( $this, 'shortcode_widget' ) );
		...
		public function tour_dates( $count = 100, $past = null, $link = null, $artist = null ) {
			...
			if ( $upcoming_shows->have_posts() ) :
				...
				while ( $upcoming_shows->have_posts() ) : $upcoming_shows->the_post();
					...
					$ticket_text   = ( $price ) ? sprintf( apply_filters( 'wolf_buy_ticket_with_price_text', __( 'Buy ticket <span class="wolf-show-ticket-price">- %s</span>', 'wolf' ) ), $price ) : apply_filters( 'wolf_buy_ticket_text', __( 'Buy ticket', 'wolf' ) );
					$artist        = get_post_meta( $post_id, '_wolf_show_artist', true );

					if ( ! $cancelled && ! $soldout && $ticket ) {

						$action = '<a target="_blank" href="' . esc_url( $ticket ) . '" class="wolf-show-ticket-button">'. $ticket_text .'</a>';
					}
					...
					if ( $free ) {
						...
						$action = '<span class="wolf-show-label free">' . __( 'free', 'wolf' ) . '</span>';
					...
					if ( $cancelled ) $display_link = false;
					...
					<tr class="wolf-single-date <?php echo sanitize_html_class( $custom_class );  ?>">
						...
						<?php if ( has_post_thumbnail( $post_id ) ) :
							$img = $this->get_post_thumbnail_url( 'full', $post_id );
		 ...
		 * @return string
		 */
		public function widget( $count, $url, $link_text, $artist, $link = false ) {
			ob_start();
			...
			if ( $upcoming_shows->have_posts() ) {
				...
				while ( $upcoming_shows->have_posts() ) : $upcoming_shows->the_post();
					...
					$free        = get_post_meta( $post_id, '_wolf_show_free', true );
					$price       = get_post_meta( $post_id, '_wolf_show_price', true );
					$ticket_text = apply_filters( 'wolf_buy_ticket_widget_text', __( 'Buy ticket', 'wolf' ) );
					$artist      = get_post_meta( $post_id, '_wolf_show_artist', true );
					$action      = '';
		...
		public function shortcode_widget( $atts ) {
			...
			return $this->widget( $count, $page_url, $link_text, $artist, $linked );
		}
